// Problem 2b Image Matching using SIFT Created by Pranav Aggarwal
...

{
  ...
  cv::Ptr<Feature2D> feature_sift = xfeatures2d::SIFT::create();
  std::vector<KeyPoint> keypoints_1, keypoints_2, selected_keypoints1, selected_keypoints2;    

  Mat descriptors_1, descriptors_2;    
 	feature_sift->detectAndCompute(vehical1, cv::noArray(), keypoints_1,descriptors_1, false);
	feature_sift->detectAndCompute(vehical2, cv::noArray(), keypoints_2,descriptors_2, false);
	double min, max;
	int min_loc;
	int count = 0;
	int best_des1[25];
  ...
  imshow("keypoints_2",vehical2_edges);
	...
	imwrite("all_matches_sift.png",rav_matches2);
    ...
    imwrite("best_matches_sift.png",rav_matches1);
