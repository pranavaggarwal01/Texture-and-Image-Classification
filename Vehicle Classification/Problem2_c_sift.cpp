// Problem 1c using sift implement BOW classification Created by Pranav Aggarwal 
#include <iostream>
#include <stdio.h>
...
/*  cv::Mat rav4_1_edges(rav4_1.size(),CV_8U);
  ...
  cv::Ptr<Feature2D> sift_features = xfeatures2d::SIFT::create();
  ...
  sift_features->detectAndCompute(rav4_2, cv::noArray(), keypoints_1,descriptors_1, false);
  sift_features->detectAndCompute(rav4_1, cv::noArray(), keypoints_2,descriptors_2, false);
  sift_features->detectAndCompute(jeep, cv::noArray(), keypoints_3,descriptors_3, false);
  sift_features->detectAndCompute(bus, cv::noArray(), keypoints_4,descriptors_4, false);
  ...
  cout << sqrt(sum1) << endl<<sqrt(sum2) <<endl<<sqrt(sum3);
  ...
  imwrite("Matched_image_sift.png", matched_image);
  imshow("Matched_image_sift", matched_image);
